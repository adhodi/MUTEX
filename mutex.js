...
(function ($) {
    function initializeToken(resource) {
        var token = {
            available: true,
            requests: new Queue(),
            owner: undefined,
            ...
            releaseToken: function (pid) {
                if (pid && pid === this.owner) {
                    ...
                    if (this.requests.contains(pid)) {
                        console.log("[TOKEN] :RELEASE FAILURE, your request for token is still in queue for " + pid);
                    } else {
                        console.log("[TOKEN] :RELEASE FAILURE, pid doesn't exists " + pid);
                    }
                }
            },
